	...
	public void bearbeiteAuftrag(Auftrag auftrag) {
		...
		if (bestandAusreichend) {
			for (Entry<Produkt, Integer> entry : auftrag.getAngebot()
				...
				hesServer.getLagerFassade().erstelleWarenausgangsmeldung(entry.getKey(),
						DateUtil.now(), entry.getValue());
			}

			Lieferung lieferung = hesServer.getLieferFassade().erstelleLieferung(auftrag);
			auftrag.setLieferung(lieferung);

			String transportDienstleister = "DHL";
			hesServer.getLieferFassade().erstelleTransportauftrag(lieferung, DateUtil.now(),
					false, DateUtil.daysFromNow(1), transportDienstleister);

			Rechnung rechnung = hesServer.getRechnungsFassade().erstelleRechnung(
